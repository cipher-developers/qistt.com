...
export async function GET(
    ...
    try {
        ...
        const remainingAmount = toTwoDecimals(Math.max(plan.sellingPrice - totalPaid, 0));
        ...
        for (let index = 0; index < 12; index += 1) {
            ...
            const remain = toTwoDecimals(Math.max(installment.amount - installment.paidAmount, 0));
            runningOutstanding = toTwoDecimals(Math.max(runningOutstanding - installment.paidAmount, 0));
        ...
        const totalReceivedSum = toTwoDecimals(
            installmentRows.reduce(
                (sum, row) => sum + (typeof row.received === "number" ? row.received : 0),
                0,
            ),
        );
        ...
        const totalOutstandingSum = toTwoDecimals(
            installmentRows.reduce(
                (sum, row) =>
                    sum + (typeof row.outstandingBalance === "number" ? row.outstandingBalance : 0),
                0,
            ),
        );
